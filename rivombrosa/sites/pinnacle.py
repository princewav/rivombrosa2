...
from rivombrosa.utilitites.utils import get_from_list_of_dicts

book = 'pinnacle'


def get_odds_by_id(straights, id):
    straight = get_from_list_of_dicts(straights, 'matchupId', id)
    if straight:
        return {'1': from_american_to_decimal(get_from_list_of_dicts(straight['prices'], 'designation', 'home').get('price') or 0),
    ...
    for matchup_info in matchups:
        if 'participants' in matchup_info:
            ...
            if not matchup_info['parent'] and all((home_team, away_team)):
                results[f'{home_team} - {away_team}'] = get_odds_by_id(straights, matchup_info['id'])
